...
<div class="card">
    <div class="card-body">
        <form action="{{ url('bpkad/surat_perintah') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <input type="hidden" name="kartu_disposisi_id" id="kartu_disposisi_id" value="{{ $ks->id }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('ditujukan') is-invalid @enderror" name="ditujukan" id="ditujukan">
                    ...
                    <option value="{{ $ks->surat->pengirim->id }}" {{ old('ditujukan') == $ks->surat->pengirim->id ? 'selected' : '' }}>
                        {{ $ks->surat->pengirim->nama_bagian }}
                    </option>
                </select>
                @error('ditujukan')
            ...
            <button type="submit" class="btn btn-success">Buat Perintah Pencairan</button>
        </form>
    </div>
</div>
